...
class PatchTST_MoE_cluster_time(nn.Module):
    def __init__(self, bs: int, c_in: int, c_out: int, context_window: int, target_window: int, patch_len: int, stride: int,
                 patch_num_in: int, patch_num_out: int,d_embed:int,rank:int,k:int|None,
                 ...
                 pe: str = 'zeros', learn_pe=True, fc_dropout: float = 0., head_dropout=0, padding_patch=None,
        ...
        self.dim = d_embed
        self.affinity_proj = nn.Linear(c_in * d_model, self.dim)
        ...
        # MoE
        self.SaprseMoE = SparseMoE(n_embed=c_in * d_model, num_experts=T_num_expert, top_k=T_top_k,d_embed=d_embed,rank=rank,k=k)
    ...
    def forward(self, z):  # z: [bs x nvars x d_model x patch_num_in]
        ...
        flat_time_z = time_z.permute(0, 3, 1, 2).reshape(bs * patch_num_out, nvars * d_model)

       
        projected_input = self.affinity_proj(flat_time_z) # [bs * patch_num_out, 64]

       
        routing_bases = self.SaprseMoE.hypernet.get_routing_bases() # [Num_Experts, 64]
        
       
        s, h = self.cluster(projected_input, routing_bases)

      
        # time_z: [bs, nvars, d_model, patch_num_out]
        time_z_moe = time_z.permute(0, 3, 1, 2)
        # Reshape to [bs, patch_num_out, nvars * d_model] for MoE sequence processing
        time_z_moe = torch.reshape(time_z_moe, (bs, patch_num_out, nvars * d_model))
        
      
        if self.training:
           
            time_z_out = self.SaprseMoE(time_z_moe, s)
            self.ortho_loss = self.SaprseMoE.ortho_loss
        else:
            
            s_reshaped = s.reshape(bs, patch_num_out, -1)
            time_z_out = self.SaprseMoE.inference(time_z_moe, s_reshaped)

        
        # time_z_out: [bs, patch_num_out, nvars * d_model]
        time_z = torch.reshape(time_z_out, (bs, patch_num_out, nvars, -1)) # [bs, patch_num_out, nvars, d_model]
        time_z = time_z.permute(0, 2, 3, 1) # [bs, nvars, d_model, patch_num_out]
