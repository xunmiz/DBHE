...
class Model(nn.Module):
    def __init__(self, configs, max_seq_len: Optional[int] = 1024, d_k: Optional[int] = None, d_v: Optional[int] = None,
        ...
        if self.decomposition:
            ...
            self.model_trend = PatchTST_MoE_cluster_time(bs=bs, c_in=c_in, c_out=c_out, context_window=context_window, target_window=target_window,
                                            patch_len=patch_len, stride=stride, patch_num_in=patch_num_in, patch_num_out=patch_num_out,d_embed=d_embed,rank=rank,k=k,
            ...
            self.model_res = PatchTST_MoE_cluster_time(bs=bs, c_in=c_in, c_out=c_out, context_window=context_window, target_window=target_window,
                                            patch_len=patch_len, stride=stride, patch_num_in=patch_num_in, patch_num_out=patch_num_out,d_embed=d_embed,rank=rank,k=k,
                                            max_seq_len=max_seq_len, n_layers=n_layers, d_model=d_model,
                                            n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                            attn_dropout=attn_dropout,
    ...
    def forward(self, x):  # x: [Batch, Input length, Channel]
        ...
        if self.decomposition:
            ...
            s_trend, h_trend, x_trend = self.model_trend(trend_init)
            s_res, h_res, x_res = self.model_res(res_init)
            s_time, s_frequency = s_trend, s_res
            output = self.head(x_trend, x_res)
        else:
    ...
    def get_ortho_loss(self):
        ...
        ortho_loss = 0.0
        if self.decomposition:
            if hasattr(self.model_trend, 'ortho_loss'):
                ortho_loss += self.model_trend.ortho_loss
            if hasattr(self.model_res, 'ortho_loss'):
                ortho_loss += self.model_res.ortho_loss
        else:
            if hasattr(self.model_time, 'ortho_loss'):
                ortho_loss += self.model_time.ortho_loss
            if hasattr(self.model_frequency, 'ortho_loss'):
                ortho_loss += self.model_frequency.ortho_loss
        return ortho_loss