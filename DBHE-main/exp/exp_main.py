...
class Exp_Main(Exp_Basic):
    ...
    def _get_model_ref(self):
        """Helper to get the underlying model (handling DataParallel)"""
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                loss_cluster_time = model_ref.model_time.cluster.total_loss(pred=s_time, target=s_tilde_time,
                                                                        beta=self.args.beta)
                loss_cluster_frequency = model_ref.model_frequency.cluster.total_loss(pred=s_frequency, target=s_tilde_frequency,
                                                                             beta=self.args.beta)

                loss_ortho = 0.01
                if hasattr(model_ref, 'get_ortho_loss'):
                    loss_ortho = model_ref.get_ortho_loss()
                ...
                # 加入 Ortho Loss (权重建议 0.01)
                loss = criterion(pred, true) + self.args.alpha * loss_cluster_time + \
                       self.args.gama * loss_cluster_frequency + \
                       self.args.ortho_loss_weight* loss_ortho
